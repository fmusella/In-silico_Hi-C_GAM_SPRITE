...
void NP_CUTTING_SINGLE_CELL(struct cluster *segregated_beads, char *cell_name)
// This function implements a proxy of the process of cutting a Nuclear Profile on a simulated cell
{
    ...
    read_cell_data(chain_1, chain_2, cell_name);
    perform_NP_cut(segregated_beads, chain_1, chain_2);
    free(chain_1);
    free(chain_2);
}

struct cluster *GAM_SINGLE_TUBE(double p_detection, char *cell_name)
// This function implements a proxy of a GAM experiment producing a single in-silico tube
{
    struct cluster *segregated_beads = initialize_cluster();
    NP_CUTTING_SINGLE_CELL(segregated_beads, cell_name);
...
// This function implements a proxy of a GAM experiment over a population of simulated cells
{
    double **segregation_table = initialize_segregation_table(N_tubes);
    int cell_index = 1 + (int) floor(drand48() * (double) N_structures / 2.);
    for(int tube_index=0; tube_index<N_tubes; tube_index++)
    {
        char cell_name[100];
        snprintf(cell_name, 100, "../data/insilico_cells/insilico_cell_%d.txt", cell_index);
        struct cluster *tube_beads = GAM_SINGLE_TUBE(p_detection, cell_name);
        fill_segregation_table(tube_index, segregation_table, tube_beads);
        free_cluster(tube_beads);
        update_cell_index(&cell_index);
    }
